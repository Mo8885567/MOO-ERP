...
/**
 * getCompanyLogoUrlForSplash — بيرجع شعار الشركة المخصص (لو موجود
 * في الكاش الجاهز) عشان نستخدمه في شاشة الـ Splash بدل الانتظار
 * لحد ما الـ JS يجيب companySettings. بيرجع "" لو مفيش كاش أو
 * مفيش شعار مخصص، فـ Index.html يرجع لـ fallback الشعار الافتراضي.
 */
function getCompanyLogoUrlForSplash() {
  try {
    var cached = _loadServerCache();
    var rawUrl =
      cached && cached.companySettings && cached.companySettings.logo_url;
    return rawUrl ? _fixDriveUrlServer(rawUrl) : "";
  } catch (e) {
    return "";
  }
}
